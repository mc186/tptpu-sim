...
Controller::Controller(MatrixMultiplyUnit *matrixmultiplyunit, VectorUnit *vectorunit, std::vector<Interconnect *> *icnt_list,
    ...
    vector_width = vectorunit->GetVectorWidth();
    // if (vector_width == 0) {
...
void Controller::VTile(int A, int B, unsigned int address_X) {
    
    int tile_width, tile_height, total_width, total_height, multiply_factor;
    unsigned int starting_address;

    int a = int( (A - 1) / vector_width) + 1; // smallest integer greater than or equal to A / vector_width
    int b = int( (B - 1) / accumulator_size) + 1;
    int remainder_a = A - ((a - 1) * vector_width);
    int remainder_b = B - ((b - 1) * accumulator_size);

    total_width = A;
    total_height = B;
    multiply_factor = 1; //(is_dimension_nchw) ? 1 : channel;    // if dimension is not nchw, it is nhwc -> multiply_factor = channel

    // push tiles: A by B
    for (int aa = 0; aa < a; ++aa) {
        for (int bb = 0; bb < b; ++bb) {
            // activations
            starting_address = address_X + multiply_factor * ((aa * vector_width * B) + (bb * accumulator_size));
            tile_height = (bb == (b - 1)) ? remainder_b : accumulator_size;
            tile_width = (aa == (a - 1)) ? remainder_a : vector_width;   
...
void Controller::PushRequestsFromVTiles() {
    std::vector<tile>::iterator it;
    std::vector<request> *ub_request_queue = vu->GetUBRequestQueue();
    float size;

    for (it = activation_tile_queue->begin(); it != activation_tile_queue->end(); ++it) {
        size = static_cast<float>(it->tile_width) * static_cast<float>(it->tile_height);
        std::cout << "Pushing tile of size " << size << " to UB request queue." << std::endl;
        ub_request_queue->push_back(MakeRequest(it->order, size));
    }
...
void Controller::VectorOp(int A, int B, unsigned int address_X) {
    // first push tiles into tile_queues
    VTile(A, B, address_X);
    std::cout << "activation_tile_queue size after VTile: " << activation_tile_queue->size() << std::endl;
    
    if (activation_tile_queue->empty()) {
        std::cerr << "Error: No tiles in activation_tile_queue to push to Vector Unit UB request queue." << std::endl;
        return;
    }


    PushRequestsFromVTiles();
    std::cout << "Vector Unit UB Request Queue size after PushRequestsFromVTiles: " << vu->GetUBRequestQueue()->size() << std::endl;
